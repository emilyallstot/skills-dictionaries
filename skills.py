    ...
    for number1 in input_list:
        for number2 in input_list:
            if number1 + number2 == 0:
                if (number1, number2) and (number2, number1) not in zero_pairs_set:
                    zero_pairs_set.add((number1, number2))
                    

    for item in zero_pairs_set:
        zero_pairs_list.append(list(item))
...
def remove_duplicates(words):
    ...
    for word in words:
        if word not in list_uniques:
            list_uniques.append(word)
...
def sort_by_word_length(words):
    ...
    for i in range(len(words)):
        key_wordcount= len(words[i])
        value_word = words[i]

        if key_wordcount not in len_words_dict:
            len_words_dict[key_wordcount] = []

        len_words_dict[key_wordcount].append(value_word)

    return len_words_dict.items()
...
def get_pirate_talk(phrase):
    ...
    """
    translation_dictionary = {'sir':'matey', 'hotel':'fleabag inn', 'student':'swabbie',
        'boy':'matey', 'madam':'proud beauty', 'professor':'foul blaggart', 'restaurant':'galley',
        'your':'yer', 'excuse':'arr', 'students':'swabbies', 'are':'be', 'lawyer':'foul blaggart',
        'the':'th', 'restroom':'head', 'my':'me', 'hello':'avast', 'is':'be', 'man':'matey'}

    original_phrase = phrase.split()
    new_phrase = []

    for word in original_phrase:
        if word in translation_dictionary.keys():
            new_phrase.append(translation_dictionary[word])
        else:
...
def adv_get_top_letter(input_string):
    ...
    for item in letter_dict:
        if item is not ' ':
            if letter_dict[item] > top_letter_count:
                top_letter = [item]
                top_letter_count = letter_dict[item]
            elif letter_dict[item] == top_letter_count:
                top_letter.append(item)
...
def adv_alpha_sort_by_word_length(words):
    ...
    for word in words:
        if len(word) not in word_length_dict:
            word_length_dict[len(word)] = []
        word_length_dict[len(word)].append(word)

    for entry_words in word_length_dict.values():
        entry_words.sort()
